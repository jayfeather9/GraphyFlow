...
Reduc_105_drain_multi_pe(hls::stream<reduce_word_t> (&pe_mem_in)[PE_NUM],
 ...
 uint32_t rounded_num_words) {
    ...
    distance_t max_val = (distance_t)(0.0);
    ap_fixed_pod_t max_pod = *reinterpret_cast<ap_fixed_pod_t *>(&max_val);
...
#pragma HLS UNROLL
            ...
            uram_res_low = (incoming_dist_pod_low == 0x0) ? uram_res_low : ( uram_res_low | incoming_dist_pod_low);
            uram_res_high = (incoming_dist_pod_high == 0x0) ? uram_res_high : ( uram_res_high | incoming_dist_pod_high);
        }
