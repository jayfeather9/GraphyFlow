...
#pragma HLS UNROLL
        ...
        // msb_out = (incoming_dist_pod != 0x0) ? (incoming_dist_pod | msb) : msb;
        // lsb_out = (incoming_dist_pod != 0x0) ? (incoming_dist_pod | lsb) : lsb;

        msb_out = (msb != 0x0) ? (incoming_dist_pod | msb) : incoming_dist_pod;
        lsb_out = (lsb != 0x0) ? (incoming_dist_pod | lsb) : incoming_dist_pod;
...
#pragma HLS UNROLL
            ...
            one_write_burst.data.range(31 + (pe_idx << 5), (pe_idx << 5)) = tmp_data[pe_idx].range(31, 0);
            one_write_burst.data.range(31 + (pe_idx << 5) + 256, (pe_idx << 5) + 256) = tmp_data[pe_idx].range(63, 32);
        }
...
#pragma HLS UNROLL
    ...
    Reduc_105_drain_multi_pe(pe_mem_out_streams, kernel_out_stream, num_word_per_pe);
}
