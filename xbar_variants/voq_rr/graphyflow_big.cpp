...
// ============================================================
// Crossbar: VOQ + per-output round-robin arbitration
//   - per-input skid buffer feeds voq[in][dst]
//   - greedy output scheduling (out 0..7), one-per-input constraint
// ============================================================
static void crossbar_voq_rr(
...
#pragma HLS UNROLL
                ...
                rr_ptr[out_id] = (ap_uint<LOG_PE_NUM>)(winner + 1);
...
#pragma HLS UNROLL
    ...
    LOOP_WHILE_XBAR_VOQ_RR_FLUSH_ENDS:
    while (true) {
#pragma HLS PIPELINE II = 1
        bool all_sent = true;
        for (int out_id = 0; out_id < PE_NUM; out_id++) {
...
#pragma HLS UNROLL
    ...
    crossbar_voq_rr(reduce_105_d2o_pair, reduce_105_o2u_pair);
