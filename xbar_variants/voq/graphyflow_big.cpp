...
// ============================================================
// VOQ depth can be tuned; keep it small for hw_emu builds unless needed.
#ifndef XBAR_VOQ_DEPTH
#define XBAR_VOQ_DEPTH 8
#endif
...
// ============================================================
// Crossbar: per-input skid buffer + per-input/per-output VOQs
//   - enqueues into voq[in][dst]
//   - greedy output arbitration (fixed priority), with one-per-input constraint
// ============================================================
static void crossbar_voq_fixed(
...
#pragma HLS ARRAY_PARTITION variable = out_streams complete dim = 0
    hls::stream<update_t_big> voq[PE_NUM][PE_NUM];
#pragma HLS STREAM variable = voq depth = XBAR_VOQ_DEPTH
#pragma HLS ARRAY_PARTITION variable = voq complete dim = 0
    ap_uint<4> voq_count[PE_NUM][PE_NUM];
#pragma HLS ARRAY_PARTITION variable = voq_count complete dim = 0
...
#pragma HLS UNROLL
        ...
        // --- VOQ enqueue phase: move buffered items into voq[in][dst] when there is space ---
        for (int in_id = 0; in_id < PE_NUM; in_id++) {
#pragma HLS UNROLL
            if (input_valid[in_id]) {
                ap_uint<LOG_PE_NUM> dst =
                    input_buf[in_id].node_id.range(LOG_PE_NUM - 1, 0);
                if (voq_count[in_id][dst] < XBAR_VOQ_DEPTH) {
                    voq[in_id][dst].write(input_buf[in_id]);
                    voq_count[in_id][dst] = (ap_uint<4>)(voq_count[in_id][dst] + 1);
        ...
        ap_uint<PE_NUM> avail[PE_NUM + 1];
#pragma HLS ARRAY_PARTITION variable = avail complete dim = 0
        avail[0] = ~ap_uint<PE_NUM>(0);
...
#pragma HLS UNROLL
            ...
            for (int in_id = 0; in_id < PE_NUM; in_id++) {
#pragma HLS UNROLL
                if (avail[out_id][in_id] && (voq_count[in_id][out_id] != 0)) {
                    req_mask[in_id] = 1;
                }
            }
            ap_uint<PE_NUM> grant_onehot = xbar_lowest_onehot(req_mask);
            ap_uint<PE_NUM> next_avail = avail[out_id];
            if ((grant_onehot != 0) && !out_streams[out_id].full()) {
                ...
                for (int in_id = 0; in_id < PE_NUM; in_id++) {
#pragma HLS UNROLL
                    if (grant_onehot[in_id]) {
                        item = voq[in_id][out_id].read();
                        voq_count[in_id][out_id] =
                            (ap_uint<4>)(voq_count[in_id][out_id] - 1);
                    }
                }
                out_streams[out_id].write(item);
                next_avail = avail[out_id] & ~grant_onehot;
            }
            avail[out_id + 1] = next_avail;
        }

        // Completion check: stop data movement only after all inputs ended and VOQs drained.
...
#pragma HLS UNROLL
            for (int out_id = 0; out_id < PE_NUM; out_id++) {
#pragma HLS UNROLL
                any_buffered |= (voq_count[in_id][out_id] != 0);
            }
        }
        done = all_inputs_closed && !any_buffered;
    }

    // Flush end tokens to each output after the data pipeline has drained.
    bool end_sent[PE_NUM];
#pragma HLS ARRAY_PARTITION variable = end_sent complete dim = 0
    for (int out_id = 0; out_id < PE_NUM; out_id++) {
#pragma HLS UNROLL
        end_sent[out_id] = false;
    }
    LOOP_WHILE_XBAR_VOQ_FLUSH_ENDS:
    while (true) {
#pragma HLS PIPELINE II = 1
        bool all_sent = true;
        for (int out_id = 0; out_id < PE_NUM; out_id++) {
#pragma HLS UNROLL
            if (!end_sent[out_id]) {
                ...
                if (out_streams[out_id].write_nb(end_item)) {
                    end_sent[out_id] = true;
                }
            }
            all_sent &= end_sent[out_id];
        }
        if (all_sent) {
            break;
        }
...
#pragma HLS UNROLL
        ...
        msb_out = (msb !=0x0) ? (((incoming_dist_pod) < (msb) ? incoming_dist_pod : msb)) : incoming_dist_pod;
        lsb_out = (lsb !=0x0) ? (((incoming_dist_pod) < (lsb) ? incoming_dist_pod : lsb)) : incoming_dist_pod;
...
#pragma HLS UNROLL
    ...
    // Replace omega network (12x switch2x2) with a single 8x8 crossbar.
    crossbar_voq_fixed(reduce_105_d2o_pair, reduce_105_o2u_pair);
