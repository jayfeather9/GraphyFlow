...
def subgraph_to_refactor() -> tuple[ComponentCollection, GlobalGraph]:
    ...
    map1 = edges.map_(map_func=lambda e: (e, e.weight + 1.0))
    # map2 is the target subgraph for our refactoring test.
    map2 = map1.map_(map_func=lambda e, const_w: (e.src.id, e.weight + const_w))
    ...
    # map2_comp_collection = map2_node.to_dfir(
