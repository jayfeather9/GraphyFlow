...
pdu = edges.map_(map_func=lambda edge: (edge.src.distance, edge.dst))

min_dist = pdu.reduce_by(
    reduce_key=lambda src_dist, dst: dst.id,
    reduce_transform=lambda src_dist, dst: (src_dist, dst),
    reduce_method=lambda x, y: (x[0] | y[0], x[1]),
)
updated_nodes = min_dist.map_(map_func=lambda dist, node: (dist | node.distance, node))
