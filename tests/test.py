...
# gather
gathered_node_property = scattered.reduce_by(reduce_key=lambda src_pr, dst: dst, reduce_method=lambda ori, update: (ori[0] + update[0], ori[1]))
# apply
apply = gathered_node_property.map_(map_func=lambda pr, node: (node, (0.15 / nodes.length + 0.85 * pr) / node.out_degree))
