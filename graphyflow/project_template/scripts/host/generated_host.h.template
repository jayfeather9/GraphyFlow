...
struct KernelBuffers {
    ...
    cl::Buffer edge_props_buf;    // Buffer for edge properties (destination ID and weight)
    cl::Buffer node_props_buf;    // Buffer for node properties (distances), updated each iteration
...
class AlgorithmHost {
  public:
    ...
    void execute_kernel_iteration(const PartitionContainer& container, std::vector<cl::Event> &big_kernel_events, std::vector<cl::Event> &little_kernel_events);
  ...
  private:
    ...
    std::vector<std::vector<KernelOutputBatch, aligned_allocator<KernelOutputBatch>>> big_kernel_host_outputs;

    // Buffer containers for little kernels (one entry per kernel instance)
    std::vector<KernelBuffers> little_kernel_buffers;
    std::vector<std::vector<KernelOutputBatch, aligned_allocator<KernelOutputBatch>>> little_kernel_host_outputs;
};
