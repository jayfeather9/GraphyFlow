...
acc_descriptor_dt initAccelerator(const std::string xclbin_path) {
    ...
    std::cout << "Attempting to program device: " << device.getInfo<CL_DEVICE_NAME>() << std::endl;
    ...
    else 
    {
        ...
        for (int i = 0; i < acc.num_big_krnl; i++) {
            ...
            std::string krnl_name_full = std::string("graphyflow_big:{") + "graphyflow_big_" + cu_id + "}";
        ...
        for (int i = 0; i < acc.num_little_krnl; i++) {
            std::string cu_id = std::to_string(i + 1);
            std::string krnl_name_full = std::string("graphyflow_little:{") + "graphyflow_little_" + cu_id + "}";
