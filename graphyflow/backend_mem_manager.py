...
class MemoryAndGraphManager:
    ...
    def validate_and_analyze(self):
        ...
        mem_read_components = [c for c in self.comp_col.components if isinstance(c, dfir.MemoryReadComponent)]
        
        print(f"[M&G Manager] Found {len(io_components)} IO, {len(reduce_components)} Reduce, {len(mem_read_components)} MemRead components.")
        ...
        assert isinstance(io_comp.output_type, ArrayType) and \
               isinstance(io_comp.output_type.type_, SpecialType) and \
               io_comp.output_type.type_.type_name == 'edge', "The single graph input must be of type Array<SpecialType('edge')>."
        ...
        assert len(mem_read_components) <= 2, "Graph must have at most two MemoryReadComponents."
        
        print("[M&G Manager] Phase 3: Classifying MemoryRead Components...")
        for mem_read in mem_read_components:
            base_types = {pattern[1] for pattern in mem_read.access_pattern}
            assert len(base_types) == 1, f"MemoryReadComponent {mem_read.readable_id} must read from a single base type (either 'node' or 'edge'), but it reads from {base_types}."
            
            base_type = base_types.pop()
            if base_type == 'edge':
                assert self.pre_reduce_mem_read is None, "Multiple MemoryReadComponents read from 'edge'. Only one is allowed."
                self.pre_reduce_mem_read = mem_read
            elif base_type == 'node':
                assert self.post_reduce_mem_read is None, "Multiple MemoryReadComponents read from 'node'. Only one is allowed."
                self.post_reduce_mem_read = mem_read
            else:
                raise ValueError(f"Invalid base type '{base_type}' in MemoryReadComponent.")
    ...
    def _define_memory_types(self):
        ...
        edge_desc_t = HLSType(HLSBasicType.STRUCT, [node_id_t, int_t], "edge_descriptor_t", ["dst_id", "weight"])
        ...
        edge_desc_batch_t = HLSType(HLSBasicType.STRUCT, [edge_desc_arr_t, int_t], "edge_descriptor_batch_t", ["edges", "end_pos"])
        
        node_id_arr_t = HLSType(HLSBasicType.ARRAY, [node_id_t], array_dims=["PE_NUM"])
        edge_batch_t = HLSType(HLSBasicType.STRUCT, [int_arr_t, int_arr_t, node_id_arr_t, int_t, bool_t], "edge_batch_t", ["weights", "src_distances", "dst_ids", "end_pos", "end_flag"])

        ibu_14_t = HLSType(HLSBasicType.STRUCT, [int_arr_t, int_t, bool_t], "struct_ibu_14_t", ["data", "end_pos", "end_flag"])
        
        # --- Type Correction: Use node_id_t for ID fields ---
        kernel_out_data_t = HLSType(HLSBasicType.STRUCT, [float_t, node_id_t], "KernelOutputData", ["distance", "id"])

        kernel_out_data_arr_t = HLSType(HLSBasicType.ARRAY, [kernel_out_data_t], array_dims=["PE_NUM"])
        kernel_out_batch_t = HLSType(HLSBasicType.STRUCT, [kernel_out_data_arr_t, bool_t, uint8_t], "KernelOutputBatch", ["data", "end_flag", "end_pos"])
        ...
        sbu_19_t = HLSType(HLSBasicType.STRUCT, [sbu_19_ele_arr_t, bool_t, uint8_t], "struct_sbu_19_t", ["data", "end_pos", "end_flag"])

        self.mem_types = {
            "edge_descriptor_t": edge_desc_t, "edge_des_burst_t": edge_des_burst_t,
            "node_distance_burst_t": node_dist_burst_t, "edge_descriptor_batch_t": edge_desc_batch_t,
            "edge_batch_t": edge_batch_t, "struct_ibu_14_t": ibu_14_t,
            "KernelOutputData": kernel_out_data_t, "KernelOutputBatch": kernel_out_batch_t,
    ...
    def _build_node_property_loader_func(self) -> HLSFunction:
        ...
        body: List[HLSCodeLine] = [
            CodeOther("const ap_uint<AXI_BUS_WIDTH> *wide_bus_ptr = reinterpret_cast<const ap_uint<AXI_BUS_WIDTH> *>(node_distances_ddr);"),
            CodeVarDecl("num_wide_reads", int_t, init_val="(num_nodes + NUM_WORDS_PER_BUS - 1) / NUM_WORDS_PER_BUS"),
        ]
        ...
        for stream_idx in range(2):
            ...
            pe_loop_body = [
                CodePragma("UNROLL"),
                CodeIf(HLSExpr(HLSExprT.VAR, HLSVar("i * NUM_WORDS_PER_BUS + j + pe < num_nodes", bool)), [
                    CodeAssign(HLSVar("burst.data[pe]", int_t), HLSExpr(HLSExprT.VAR, HLSVar("wide_word.range((j + pe + 1) * DATA_TYPE_WIDTH - 1, (j + pe) * DATA_TYPE_WIDTH)", int_t))),
                ]),
            ]
            ...
            middle_if = CodeIf(
                ...
                [
                    CodeWriteStream(HLSVar(f"node_distance_burst_stream_{stream_idx}", stream_t), HLSVar("burst", T["node_distance_burst_t"])),
                    CodeAssign(HLSVar("sent_pack_cnt", int_t), HLSExpr(HLSExprT.VAR, HLSVar("sent_pack_cnt + 1", int_t)))
                ]
            )
            ...
            main_loop_body = [
                ...
                CodeAssign(HLSVar("wide_word", HLSType(HLSBasicType.AP_UINT, width="AXI_BUS_WIDTH")), HLSExpr(HLSExprT.VAR, HLSVar("wide_bus_ptr[i]", HLSType(HLSBasicType.AP_UINT, width="AXI_BUS_WIDTH")))),
    ...
    def _build_edge_descriptor_loader_func(self) -> HLSFunction:
        ...
        func.params = [
            HLSVar("edge_des_bursts", HLSType(HLSBasicType.POINTER, [T["edge_des_burst_t"]], is_const_ptr=True)),
        ...
        inner_loop = CodeFor([
            CodePragma("UNROLL"),
            CodeIf(HLSExpr(HLSExprT.VAR, HLSVar("i * PE_NUM + j < num_edges", bool)), [
                CodeAssign(HLSVar("edge_batch.edges[edge_batch.end_pos]", T["edge_descriptor_t"]), HLSExpr(HLSExprT.VAR, HLSVar("burst.edges[j]", T["edge_descriptor_t"]))),
                CodeAssign(HLSVar("edge_batch.end_pos", HLSType(HLSBasicType.INT)), HLSExpr(HLSExprT.VAR, HLSVar("edge_batch.end_pos + 1", HLSType(HLSBasicType.INT))))
            ])
        ], "PE_NUM", iter_name="j")
        ...
        main_loop = CodeFor([
            ...
            CodeAssign(HLSVar("burst", T["edge_des_burst_t"]), HLSExpr(HLSExprT.VAR, HLSVar("edge_des_bursts[i]", T["edge_des_burst_t"]))),
            CodeAssign(HLSVar("edge_batch.end_pos", HLSType(HLSBasicType.INT)), HLSExpr(HLSExprT.CONST, 0)),
            inner_loop,
            CodeWriteStream(func.params[1], HLSVar("edge_batch", T["edge_descriptor_batch_t"]))
        ], "(num_edges + PE_NUM - 1) / PE_NUM", iter_name="i")
    ...
    def _build_src_offset_loader_func(self) -> HLSFunction:
        ...
        inner_if = CodeIf(
            ...
            [
                CodeVarDecl("cur_data", int_t, init_val="wide_word.range((j + 1) * DATA_TYPE_WIDTH - 1, j * DATA_TYPE_WIDTH)"),
        ...
        main_loop = CodeFor([
            ...
            CodeAssign(HLSVar("wide_word", HLSType(HLSBasicType.AP_UINT, width="AXI_BUS_WIDTH")), HLSExpr(HLSExprT.VAR, HLSVar("wide_bus_ptr[i]", HLSType(HLSBasicType.AP_UINT, width="AXI_BUS_WIDTH")))),
            inner_loop
        ], "(num_nodes + 1 + NUM_WORDS_PER_BUS - 1) / NUM_WORDS_PER_BUS", iter_cmp="<=", iter_name="i")

        func.codes = [
            CodeOther("const ap_uint<AXI_BUS_WIDTH> *wide_bus_ptr = reinterpret_cast<const ap_uint<AXI_BUS_WIDTH> *>(src_offsets_ddr);"),
    ...
    def _build_edge_property_loader_and_dispatcher_func(self) -> HLSFunction:
        ...
        edge_loop = CodeFor([
            CodePragma("PIPELINE II=1"),
            CodeIf(HLSExpr(HLSExprT.VAR, HLSVar("edge_batch_pos == edge_batch.end_pos", bool)), [
                CodeAssign(HLSVar("edge_batch", T["edge_descriptor_batch_t"]), HLSExpr(HLSExprT.STREAM_READ, None, [HLSExpr(HLSExprT.VAR, func.params[1])])),
                CodeAssign(HLSVar("edge_batch_pos", int_t), HLSExpr(HLSExprT.CONST, 0))
            ]),
            CodeAssign(HLSVar("edge", T["edge_descriptor_t"]), HLSExpr(HLSExprT.VAR, HLSVar("edge_batch.edges[edge_batch_pos++]", T["edge_descriptor_t"]))),
            CodeAssign(HLSVar("current_batch.weights[current_batch.end_pos]", int_t), HLSExpr(HLSExprT.VAR, HLSVar("edge.weight", int_t))),
            CodeAssign(HLSVar("current_batch.src_distances[current_batch.end_pos]", int_t), HLSExpr(HLSExprT.VAR, HLSVar("src_dist", int_t))),
            CodeAssign(HLSVar("current_batch.dst_ids[current_batch.end_pos]", HLSType(HLSBasicType.NODE_ID)), HLSExpr(HLSExprT.VAR, HLSVar("edge.dst_id", HLSType(HLSBasicType.NODE_ID)))),
            CodeAssign(HLSVar("current_batch.end_pos", int_t), HLSExpr(HLSExprT.VAR, HLSVar("current_batch.end_pos + 1", int_t))),
            CodeIf(HLSExpr(HLSExprT.VAR, HLSVar("current_batch.end_pos == PE_NUM", bool)), [
        ...
        ], "end_edge_idx", iter_name="e_idx", iter_cmp="<")
        # Custom loop start
        edge_loop.iter_start = "start_edge_idx"

        pe_loop = CodeFor([
            CodeIf(HLSExpr(HLSExprT.VAR, HLSVar("base_idx + pe_idx >= num_nodes", bool)), [CodeBreak()]),
            CodeAssign(HLSVar("src_dist", int_t), HLSExpr(HLSExprT.VAR, HLSVar("node_distance_burst.data[pe_idx]", int_t))),
            CodeAssign(HLSVar("end_edge_idx", int_t), HLSExpr(HLSExprT.STREAM_READ, None, [HLSExpr(HLSExprT.VAR, func.params[0])])),
            edge_loop,
            CodeAssign(HLSVar("start_edge_idx", int_t), HLSExpr(HLSExprT.VAR, HLSVar("end_edge_idx", int_t))),
        ], "PE_NUM", iter_name="pe_idx")

        main_loop = CodeFor([
            CodeAssign(HLSVar("node_distance_burst", T["node_distance_burst_t"]), HLSExpr(HLSExprT.STREAM_READ, None, [HLSExpr(HLSExprT.VAR, func.params[2])])),
            pe_loop
        ], "(num_nodes + PE_NUM - 1) / PE_NUM", iter_name="node_burst_idx")
        ...
        func.codes = [
            ...
            CodeAssign(HLSVar("start_edge_idx", int_t), HLSExpr(HLSExprT.STREAM_READ, None, [HLSExpr(HLSExprT.VAR, func.params[0])])),
            main_loop,
    ...
    def _build_node_property_responder_func(self) -> HLSFunction:
        ...
        pe_loop = CodeFor([
            CodePragma("UNROLL"),
            CodeIf(HLSExpr(HLSExprT.VAR, HLSVar("base_idx + pe_idx >= num_nodes", bool)), [CodeBreak()]),
            CodeAssign(HLSVar("dist_batch.data[dist_batch.end_pos]", HLSType(HLSBasicType.INT)), HLSExpr(HLSExprT.VAR, HLSVar("node_distance_burst.data[pe_idx]", HLSType(HLSBasicType.INT)))),
            CodeAssign(HLSVar("dist_batch.end_pos", HLSType(HLSBasicType.INT)), HLSExpr(HLSExprT.VAR, HLSVar("dist_batch.end_pos + 1", HLSType(HLSBasicType.INT)))),
        ], "PE_NUM", iter_name="pe_idx")

        main_loop = CodeFor([
            CodePragma("PIPELINE II=1"),
            CodeAssign(HLSVar("node_distance_burst", T["node_distance_burst_t"]), HLSExpr(HLSExprT.STREAM_READ, None, [HLSExpr(HLSExprT.VAR, func.params[0])])),
            pe_loop,
    ...
    def _build_final_convert_func(self) -> HLSFunction:
        ...
        loop = CodeFor([
            CodePragma("UNROLL"),
            CodeOther("ap_fixed<32, 16> dist_fp = *reinterpret_cast<ap_fixed<32, 16> *>(&in_batch.data[i].ele_0);"),
            CodeAssign(HLSVar("out_batch.data[i].distance", HLSType(HLSBasicType.REAL_FLOAT)), HLSExpr(HLSExprT.VAR, HLSVar("(float)dist_fp", HLSType(HLSBasicType.REAL_FLOAT)))),
            CodeAssign(HLSVar("out_batch.data[i].id", HLSType(HLSBasicType.NODE_ID)), HLSExpr(HLSExprT.VAR, HLSVar("in_batch.data[i].ele_1", HLSType(HLSBasicType.NODE_ID)))),
        ], "PE_NUM", iter_name="i")

        func.codes = [CodeWhile(codes=[
            CodePragma("PIPELINE II=1"),
            CodeAssign(HLSVar("in_batch", T["struct_sbu_19_t"]), HLSExpr(HLSExprT.STREAM_READ, None, [HLSExpr(HLSExprT.VAR, func.params[0])])),
            CodeVarDecl("out_batch", T["KernelOutputBatch"]),
            CodeAssign(HLSVar("out_batch.end_flag", bool), HLSExpr(HLSExprT.VAR, HLSVar("in_batch.end_flag", bool))),
            CodeAssign(HLSVar("out_batch.end_pos", int), HLSExpr(HLSExprT.VAR, HLSVar("in_batch.end_pos", int))),
            loop,
    ...
    def _build_final_write_func(self) -> HLSFunction:
        ...
        func.codes = [
            ...
            CodeWhile(codes=[
                CodePragma("PIPELINE II=1"),
                CodeAssign(HLSVar("out_batch", T["KernelOutputBatch"]), HLSExpr(HLSExprT.STREAM_READ, None, [HLSExpr(HLSExprT.VAR, func.params[0])])),
                CodeAssign(HLSVar("out_o_0_342[i]", T["KernelOutputBatch"]), HLSExpr(HLSExprT.VAR, HLSVar("out_batch", T["KernelOutputBatch"]))),
    ...
    def _build_memory_loader_func(self) -> HLSFunction:
        ...
        params = [
            ...
            HLSVar("src_offsets", HLSType(HLSBasicType.POINTER, [HLSType(HLSBasicType.INT)], is_const_ptr=True)),
            HLSVar("edge_des_bursts", HLSType(HLSBasicType.POINTER, [T["edge_des_burst_t"]], is_const_ptr=True)),
            HLSVar("node_distances", HLSType(HLSBasicType.POINTER, [HLSType(HLSBasicType.INT)], is_const_ptr=True)),
            HLSVar("num_nodes", HLSType(HLSBasicType.INT)),
            HLSVar("num_edges", HLSType(HLSBasicType.INT)),
        ...
        node_prop_loader, edge_desc_loader, src_offset_loader, edge_prop_dispatcher, node_prop_responder, _, _ = self.helper_funcs

        body.extend([
            CodeCall(src_offset_loader, [params[1], HLSVar("src_offsets_cache_stream", stream_decls[3][1]), params[4]]),
            CodeCall(node_prop_loader, [params[3], HLSVar("node_distance_burst_stream_0", stream_decls[0][1]), HLSVar("node_distance_burst_stream_1", stream_decls[1][1]), params[4]]),
            CodeCall(edge_desc_loader, [params[2], HLSVar("edge_stream", stream_decls[2][1]), params[5]]),
            CodeCall(edge_prop_dispatcher, [HLSVar("src_offsets_cache_stream", stream_decls[3][1]), HLSVar("edge_stream", stream_decls[2][1]), HLSVar("node_distance_burst_stream_0", stream_decls[0][1]), params[4], params[6]]),
            CodeCall(node_prop_responder, [HLSVar("node_distance_burst_stream_1", stream_decls[1][1]), params[4], params[7]]),
        ])
