...
class Component(DfirNode):
    def __init__(self, input_type: DfirType, output_type: DfirType, ports: List[str], parallel: bool = False, specific_port_types: Optional[Dict[str, DfirType]] = None) -> None:
        super().__init__()
...
class UnaryOpComponent(Component):
    def __init__(self, op: UnaryOp, input_type: DfirType, select_index: Optional[int] = None) -> None:
        if isinstance(input_type, ArrayType):
            parallel = True
            output_type = ArrayType(op.output_type(input_type.type_))
...
class ConditionalComponent(Component):
    def __init__(self, input_type: DfirType) -> None:
        if isinstance(input_type, ArrayType):
            ...
            cond_type = BoolType()
        super().__init__(input_type, output_type, ["i_data", "i_cond", "o_0"], parallel, {"i_cond": cond_type})

class CollectComponent(Component):
    def __init__(self, input_type: DfirType) -> None:
        output_type = ArrayType(input_type) if not isinstance(input_type, OptionalType) else ArrayType(input_type.type_)
