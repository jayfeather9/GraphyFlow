...
LOOP_APPLY:
    for (uint32_t addr = 0; addr < dst_num; addr += DIST_PER_WORD) {
#pragma HLS PIPELINE II = 1
        ...
        for (int i = 0; i < DIST_PER_WORD; i++) {
#pragma HLS UNROLL
            ap_fixed_pod_t update_dist = wide_word.range(
                DISTANCE_BITWIDTH - 1 + (i << LOG_DIST_BITWIDTH),
                (i << LOG_DIST_BITWIDTH));
            ap_fixed_pod_t current_dist = node_prop.range(
                DISTANCE_BITWIDTH - 1 + (i << LOG_DIST_BITWIDTH),
                (i << LOG_DIST_BITWIDTH));
            ap_fixed_pod_t new_dist =
                (update_dist < current_dist) ? update_dist : current_dist;
            new_node_prop.range(DISTANCE_BITWIDTH - 1 +
                                    (i << LOG_DIST_BITWIDTH),
                                (i << LOG_DIST_BITWIDTH)) = new_dist;
        }
