...
#pragma HLS UNROLL
                ...
                msb_out =
                    (msb != 0x0)
                        ? (((msb) < (incoming_dist_pod) ? msb
                                                        : incoming_dist_pod))
                        : incoming_dist_pod;
                lsb_out =
                    (lsb != 0x0)
                        ? (((lsb) < (incoming_dist_pod) ? lsb
                                                        : incoming_dist_pod))
                        : incoming_dist_pod;
...
static void Reduc_105_partial_drain_impl(
    int32_t i, hls::stream<reduce_word_t> (&pe_mem_in)[4],
...
#pragma HLS UNROLL
            ...
            // =======  begin inline reduce logic ====
            uram_res_low = (incoming_dist_pod_low != 0x0)
                               ? (((incoming_dist_pod_low) < (uram_res_low)
                                       ? incoming_dist_pod_low
                                       : uram_res_low))
                               : uram_res_low;
            uram_res_high = (incoming_dist_pod_high != 0x0)
                                ? (((incoming_dist_pod_high) < (uram_res_high)
...
#pragma HLS PIPELINE II = 1
        ...
        first_low =
            (second_low != 0x0)
                ? (((second_low) < (first_low) ? second_low : first_low))
                : first_low;
        first_high =
            (second_high != 0x0)
                ? (((second_high) < (first_high) ? second_high : first_high))
                : first_high;
        // =======  end inline reduce logic ====
...
#pragma HLS UNROLL
    ...
    // --- Reduction ---
    Reduc_105_unit_reduce(stream_edge_data, pe_mem_outs_1, pe_mem_outs_2,
