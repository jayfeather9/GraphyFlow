...
extern "C" void graphyflow_little( 
                                    ...
                                    hls::stream<l_ppb_request_pkt> &l_ppb_request_stm,   // <--- 修改: 使用 AXI Stream 包类型
                                    hls::stream<l_ppb_response_pkt> &l_ppb_response_stm, // <--- 修改: 使用 AXI Stream 包类型
                                    hls::stream<b_node_distance_burst_t> &stream_outer_node_dist
                                ) {
...
#pragma HLS UNROLL
    ...
    stream2axistream<ppb_request_dt,l_ppb_request_pkt>(ppb_request_stm, l_ppb_request_stm);
    axistream2stream<l_ppb_response_pkt,ppb_response_dt>(l_ppb_response_stm, ppb_response_stm);
