...
void AlgorithmHost::setup_buffers(const GraphCSR &graph, int start_node) {
    ...
    OCL_CHECK(err, d_i_0_edge_id_320 = cl::Buffer(
                       m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                       h_i_0_edge_id_320.size() * sizeof(struct_ebu_4_t),
                       h_i_0_edge_id_320.data(), &err));
...
void AlgorithmHost::transfer_data_to_fpga() {
    ...
    OCL_CHECK(err, err = m_q.enqueueMigrateMemObjects(
                       {d_i_0_edge_id_320, d_stop_flag}, 0));
}
...
void AlgorithmHost::transfer_data_from_fpga() {
    cl_int err;
    OCL_CHECK(err, err = m_q.enqueueMigrateMemObjects(
                       {d_o_0_342, d_stop_flag}, CL_MIGRATE_MEM_OBJECT_HOST));
...
bool AlgorithmHost::check_convergence_and_update() {
    ...
    if (changed) {
        for (auto &batch : h_i_0_edge_id_320) {
            for (int i = 0; i < batch.end_pos; ++i) {
                batch.data[i].src.distance =
                    ap_fixed_to_int32(h_distances[batch.data[i].src.id]);
                batch.data[i].dst.distance =
                    ap_fixed_to_int32(h_distances[batch.data[i].dst.id]);
            }
        }
    }
