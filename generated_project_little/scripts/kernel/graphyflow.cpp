...
void stream_zipper_0(hls::stream<struct_ibu_10_t> &in_key_batch_stream, hls::stream<struct_sbu_21_t> &in_transform_batch_stream, hls::stream<struct_kbu_27_t> &out_pair_batch_stream) {
    struct_ibu_10_t key_batch;
    struct_sbu_21_t transform_batch;
    struct_kbu_27_t out_batch;
    while (true) {
#pragma HLS PIPELINE
...
void Reduc_141_unit_reduce(hls::stream<struct_kbu_27_t> &in_kt_pair_stream, hls::stream<struct_sbu_21_t> &o_0) {
    ...
    // 1a. Partitioned stateful memories for PE_NUM parallel reduction units
    struct_sb_34_t key_mem[PE_NUM][MAX_NUM];
#pragma HLS BIND_STORAGE variable=key_mem type=RAM_2P impl=URAM
#pragma HLS ARRAY_PARTITION variable=key_mem complete dim=1
    struct_sb_34_t key_buffer[PE_NUM][L + 1];
...
#pragma HLS ARRAY_PARTITION variable=i_buffer complete dim=0
    // 1b. Memory initialization for all PEs
...
#pragma HLS UNROLL
    ...
    // 1c. Main processing loop reading batches and processing in parallel
    struct_kbu_27_t in_batch;
    while (true) {
#pragma HLS PIPELINE
        in_batch = in_kt_pair_stream.read();
        for (uint32_t i = 0; i < PE_NUM; i++) {
#pragma HLS UNROLL
            int32_t pe_key;
            struct_in_19_t pe_val;
            pe_key = in_batch.data[i].key;
            pe_val = in_batch.data[i].transform;
            if ((i < in_batch.end_pos)) {
                struct_sb_34_t old_ele;
                old_ele = key_mem[i][pe_key];
                for (uint32_t i_search = 0; i_search < L + 1; i_search++) {
#pragma HLS UNROLL
                    if ((pe_key == i_buffer[i][i_search])) {
...
#pragma HLS UNROLL
                ...
                if (old_ele.ele_1) {
                    ...
                    temp_Unary_129_o_0 = pe_val.ele_0;
                    ...
                    new_ele.ele_1 = true;
                    new_ele.ele_0 = pe_val;
                }
                key_mem[i][pe_key] = new_ele;
    ...
    // 2a. Prepare for batching the final results
    uint8_t out_idx = 0;
    struct_sbu_21_t data_pack;
    // 2b. Iterate through all keys, merge results from all PEs, and stream out
    for (uint32_t addr = 0; addr < MAX_NUM; addr++) {
#pragma HLS PIPELINE
        struct_in_19_t final_value;
        bool is_valid_addr = false;
        for (uint32_t pe = 0; pe < PE_NUM; pe++) {
#pragma HLS UNROLL
            struct_sb_34_t partial_result;
            partial_result = key_mem[pe][addr];
            if (partial_result.ele_1) {
                if ((!is_valid_addr)) {
                    is_valid_addr = true;
                    final_value = partial_result.ele_0;
                } else {
                    struct_in_19_t partial_data;
                    partial_data = partial_result.ele_0;
                    // -- Inline sub graph --
                    // Starting for comp CopyC_125
                    struct_in_19_t temp_CopyC_125_o_0;
                    struct_in_19_t temp_CopyC_125_o_1;
                    temp_CopyC_125_o_0 = final_value;
                    temp_CopyC_125_o_1 = final_value;
                    // Starting for comp Unary_129
                    int32_t temp_Unary_129_o_0;
                    temp_Unary_129_o_0 = partial_data.ele_0;
                    // Starting for comp Unary_119
                    int32_t temp_Unary_119_o_0;
                    temp_Unary_119_o_0 = temp_CopyC_125_o_0.ele_0;
                    // Starting for comp Unary_122
                    ...
                    temp_BinOp_132_o_0 = *reinterpret_cast<int32_t*>(&temp_BinOp_132_o_0_ap_result);
                    // Starting for comp Gathe_136
                    final_value.ele_0 = temp_BinOp_132_o_0;
                    final_value.ele_1 = temp_Unary_122_o_0;
                    // -- Inline sub graph end --
                }
            }
        }
        if (is_valid_addr) {
            data_pack.data[out_idx] = final_value;
            out_idx = (out_idx + 1);
            if ((out_idx == PE_NUM)) {
                data_pack.end_pos = out_idx;
    ...
    // 2c. Drain any remaining data and send final batch with end_flag
    if ((out_idx > 0)) {
        data_pack.end_pos = out_idx;
        data_pack.end_flag = false;
        o_0.write(data_pack);
...
#pragma HLS DATAFLOW
    hls::stream<struct_kbu_27_t> reduce_141_z2u_pair;
#pragma HLS STREAM variable=reduce_141_z2u_pair depth=4
...
#pragma HLS STREAM variable=stream_o_1_171 depth=4
    ...
    // --- Start of Reduce Super-Block for Reduc_141 (Little Pipeline) ---
    Reduc_141_pre_process(stream_o_0_67, intermediate_key, intermediate_transform);
    stream_zipper_0(intermediate_key, intermediate_transform, reduce_141_z2u_pair);
    Reduc_141_unit_reduce(reduce_141_z2u_pair, stream_o_0_143);
